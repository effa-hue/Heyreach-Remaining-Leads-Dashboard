...
const CLIENTS = [
  {
    ...
    followUp: {
      ...
      // `Addressed` is what the Slack button writes; it belongs here for the same reason
      // the other two do -- a tag is how a lead stops being chased.
      excludeTags: ['Not interested', 'Disqualified on facts', 'Addressed'],
      // Written to the lead when a sender presses Addressed. Must exist in the HeyReach
      // workspace and must appear in `excludeTags`, or the button resolves the Slack
      // message without stopping the next step from firing.
