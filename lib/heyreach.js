...
async function hrGet(apiKey, path, { retries = 2 } = {}) {
  let lastErr;
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      const res = await fetch(`${BASE}${path}`, {
...
/** Steps that send a message rather than a connection request. */
const MESSAGING_NODES = new Set(['MESSAGE', 'INMAIL', 'MESSAGE_INMAIL', 'VOICE_NOTE']);

/**
 * Does this sequence open by sending a connection request?
 *
 * Only campaigns that do can consume a sender's daily connection-request budget. First-degree
 * campaigns target people the sender is already connected to and open straight at MESSAGE, so
 * their queued leads must not be counted as connection-request inventory.
 *
 * Walks from the root and returns on the first send step it reaches: CONNECTION_REQUEST means
 * yes, a messaging step means no. `CHECK_IS_CONNECTION` is a gate rather than a send, so both
 * of its branches are followed — the house pattern puts the connection request on the
 * unconditional (not-yet-connected) side and END on the other.
 */
function sendsConnectionRequests(node) {
  if (!node || typeof node !== 'object') return false;
  const type = node.nodeType;
  if (type === 'CONNECTION_REQUEST') return true;
  if (MESSAGING_NODES.has(type)) return false;
  return (
    sendsConnectionRequests(node.conditionalNode) || sendsConnectionRequests(node.unconditionalNode)
  );
}

/** The campaign's step tree. Reusable directly as an UpdateSequence payload. */
async function getSequence(apiKey, campaignId) {
  return hrGet(apiKey, `/campaign/GetCampaignSequence?campaignId=${encodeURIComponent(campaignId)}`);
}
...
async function getActiveCampaigns(apiKey) {
  ...
  return (json.items ?? [])
    ...
    .map((c) => ({
      ...
      // Leads added to the campaign that have not entered the sequence yet. These are NOT
      // returned by GetLeadsFromCampaign at all -- totalUsers minus the number that endpoint
      // lists equals exactly this figure on every campaign checked -- so they are invisible to
      // any per-sender tally and have to be picked up here.
...
module.exports = {
  hrPost, hrGet, mapLimit, getSenders, getActiveCampaigns, getQueuedBySender, getSentToday,
  getSequence, sendsConnectionRequests, PAGE_SIZE,
};

/**
 * Add tags to a lead.
 *
 * Note the parameter names differ from `lead/GetTags`, which wants `ProfileUrl`: this one
 * insists on `leadProfileUrl` (or `leadLinkedInId`) and 400s with
 * "One of the following parameters should be provided" if given the other spelling.
 *
 * Additive. There is no RemoveTags endpoint -- `lead/ReplaceTags` is the only way back, and
 * it overwrites the whole list, so read the current tags first if you ever undo this.
 *
 * Whether this creates a tag that does not yet exist in the workspace is UNVERIFIED -- it
 * could not be established without writing to a real lead in a client workspace. Create
 * the tags in the HeyReach UI first and the question does not arise; see SETUP.md.
 */
async function addLeadTags(apiKey, { profileUrl, linkedInId, tags }) {
  const body = { tags };
  // linkedInId is the stabler handle -- a lead can change their LinkedIn vanity URL, and
  // profileUrl then stops matching.
  if (linkedInId) body.leadLinkedInId = String(linkedInId);
  else body.leadProfileUrl = profileUrl;
  return hrPost(apiKey, '/lead/AddTags', body);
}

module.exports.addLeadTags = addLeadTags;
