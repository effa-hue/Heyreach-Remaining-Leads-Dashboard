...
function senderTable(senders) {
  // `Assgnd` is the queue HeyReach has already attached to this sender; `+New` is its
  // estimated share of leads added but not yet handed out. Splitting the column keeps the
  // estimated part visible instead of blending it into a figure that looks exact.
  const anyPending = senders.some((s) => s.pendingShare > 0);
  const head = anyPending
    ? `${padR('Sender', 16)}${padL('Lim', 5)}${padL('Sent', 6)}${padL('Assgnd', 8)}${padL('+New', 6)}${padL('Re-up', 7)}`
  ...
  for (const s of [...senders].sort(byUrgency)) {
    ...
    rows.push(
      anyPending
        ? `${padR(s.name.slice(0, 15), 16)}${padL(s.dailyLimit, 5)}${padL(s.sentToday, 6)}${padL(s.assignedQueued ?? s.queued, 8)}${padL(s.pendingShare ? '+' + s.pendingShare : '-', 6)}${padL(reup, 7)}`
 ...
 */
function buildMessage({ assessment, state, run, postAllClear = false, redirectedFrom = null }) {
  const { client, senders, atRisk, blocked, totals, campaigns, excluded = [], pendingTotal = 0 } = assessment;
      ...
      blocks: [
        section(
          `🟢 *${label} — quota covered*\n` +
            ...
            (pendingTotal > 0 ? `\n_Includes an even-split estimate of ${fmt(pendingTotal)} lead${pendingTotal === 1 ? '' : 's'} HeyReach is still assigning._` : '')
        ),
  ...
  const footer = [];
  if (pendingTotal > 0) {
    footer.push(`${fmt(pendingTotal)} lead${pendingTotal === 1 ? '' : 's'} still being assigned by HeyReach, split evenly as an estimate`);
  }
  // Say where this would normally have gone, so a redirected alert is not mistaken for the
  // client channel having been notified.
  if (redirectedFrom) footer.push(`would normally post to ${redirectedFrom}`);
  if (excluded.length) {
