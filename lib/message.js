...
function buildMessage({ assessment, state, run, postAllClear = false }) {
  const { client, senders, atRisk, blocked, totals } = assessment;
  ...
  const context =
    run === 'preflight' && state.beforeWindow
      ...
      : `${humanDuration(state.minutesToEnd)} left, ${state.elapsedPct}% through · *${totals.sentToday} of ${totals.dailyLimit}* sent${shortOf}`;

  const blocks = [section(`${headline}\n${context}`)];

  if (atRisk.length > 0) {
    blocks.push(section([...atRisk].sort(byUrgency).map(senderLine).join('\n')));
  }
  ...
  if (process.env.DASHBOARD_URL) {
    blocks.push(context_(`<${process.env.DASHBOARD_URL}|Open the dashboard>`));
  }
...
/** "• *Sam Grasso* — *23* short · nothing queued in 16 campaigns" */
function senderLine(s) {
  const where =
    s.campaigns.length === 0
      ? `nothing queued in ${s.activeCampaigns} campaign${s.activeCampaigns === 1 ? '' : 's'}`
