...
const { readAddressed, leadKey } = require('./store');
...
/**
 * Should a lead a sender already marked Addressed be chased again?
 *
 * Only if they have written to us *since* it was marked. Someone pressing Addressed is
 * saying "this one is handled" -- but a lead who then replies has un-handled it, and that
 * reply is exactly the thing the tracker exists to stop us dropping.
 *
 * The comparison needs the moment the button was pressed, which is the single fact the
 * HeyReach tag cannot carry (see lib/store.js). Two fallbacks matter:
 *
 *   - no store configured (`addressedAt === null`) -- treat the tag as a permanent
 *     exclusion, the behaviour before the store existed. Never chase on a guess.
 *   - store configured but no entry for this lead -- someone applied the tag by hand in
 *     the HeyReach UI, deliberately and outside this system. Respect it, permanently.
 */
function isRevived(thread, addressedAt) {
  if (addressedAt === null) return false;
  const at = addressedAt[leadKey(thread)];
  if (at === undefined) return false;
  return thread.lastInboundAt > at;
}

async function getWinnableThreads(client, env = process.env) {
  const { items, missingTags } = await getTaggedConversations(
    client.apiKey,
    client.followUp.winnableTags
  );

  // A store outage must not turn into a flood of reminders for leads people already
  // handled, so a failed read degrades to "no store" rather than to "nothing addressed".
  let addressedAt = null;
  let storeError = null;
  try {
    addressedAt = await readAddressed(client.key, env);
  } catch (err) {
    storeError = err.message;
    console.error(`[inbox] addressed-store read failed for ${client.key}:`, err);
  }

  const addressedTag = client.followUp.addressedTag;
  const threads = items
    .map(toThread)
    .filter(Boolean)
    .filter((t) => !t.tags.some((tag) => client.followUp.excludeTags.includes(tag)))
    .filter((t) => !t.tags.includes(addressedTag) || isRevived(t, addressedAt))
    .sort((a, b) => b.lastMessageAt - a.lastMessageAt);

  return { threads, missingTags, storeError, storeConfigured: addressedAt !== null };
}
